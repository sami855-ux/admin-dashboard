  ...
  },
  {
    ...
    cell: ({ row }) => [
      h(
        ...
        {
          class:
            "p-button p-button-sm p-button-warning text-green-700 mr-4 cursor-pointer",
  ...
  <div class="w-full h-fit">
    <div class="w-fit h-16 flex items-center gap-3">
      ...
      <p class="text-semibold text-[16px] py-4">
        Dashboard - Completed Transaction
      </p>
    </div>
    <div class="w-full h-10 flex justify-end items-center lg:pr-16">
      <button
        class="px-7 text-custom text-[14px] py-1 bg-blue-400 border border-blue-200 rounded-md cursor-pointer"
        @click="handleModal"
      >
        Create Employee +
      </button>
    ...
    <div class="w-full h-fit">
      <div class="w-full min-h-10 flex items-center justify-between lg:pr-16">
        ...
        <div class="h-full w-fit pl-14 lg:pl-0 lg:ml-5">
          <div class="w-full flex gap-5 items-center justify-self-end">
            ...
            <div
              ...
              class="absolute miniModal top-[210px] w-48 bg-custom rounded-md border border-custom h-fit p-5 right-10 lg:right-20 transition duration-150 ease-in-out"
            >
              <p
    ...
    <div
      :class="` ${
        isModalOpen || isEditModalOpen
          ? 'opacity-100 visible'
          : 'opacity-0 invisible'
      } w-full h-full fixed top-0 left-0 z-80 bg-gray-700/50 transition duration-150 ease-in-out cursor-pointer`"
      @click="handleModal"
    ></div>
    <div
      :class="` ${
        isModalOpen ? 'right-0' : 'right-[-200%]'
      } w-[400px] lg:w-[600px] h-screen fixed top-0  z-100 bg-color py-4 px-10 border border-custom  transition duration-500 ease-in-out overflow-scroll`"
    >
      <i class="pi pi-times cursor-pointer" @click="handleModal"></i>

      <h2 class="text-lg font-semibold py-1 mt-1 text-custom">
    ...
    <div
      ...
      } w-[600px] hidden lg:block h-screen fixed top-0  z-100 bg-color p-4 border border-custom  transition duration-500 ease-in-out overflow-scroll`"
    >
      <i
